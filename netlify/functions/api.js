...
// ── JWT ───────────────────────────────────────────────────────────────────────
const JWT_SECRET = process.env.JWT_SECRET || "hsse-dan-secret-CHANGE-IN-PRODUCTION";
...
// Both "weeklyReports" (camelCase, used by the React client) and
// "weekly-reports" (kebab-case, historical) map to the weekly_reports table.
...
async function handlePut(collection, id, event, user) {
  ...
  try {
    ...
    // Split incoming fields into (a) whitelisted real columns and
    // (b) everything else, which is merged into the raw JSONB blob so nothing
    // is silently dropped. Previously, any mix of known + unknown fields meant
    // the unknown ones vanished entirely.
    const updates = [];
    const rawExtras = {};
    ...
    if (updates.length === 0) {
      // No real columns to set — just merge everything into raw
      await pool.query(
    ...
    // Always append a raw-merge update so unknown fields survive.
...
async function insUser(pool, d) {
  ...
  const hash = d.password ? crypto.createHash("sha256").update(d.password).digest("hex") : null;
  await pool.query(`
